...
filePathForTable="${gitDir}/prepare/table_genomes.txt"

# STAR index may be incompatible between versions
# Therefore it may be interesting to keep a record of the STAR version used
# Substitute the name of your genome by __genome__
dirPathForSTARIndex="${genomeDirectory}/STARIndex_2.7.10a/__genome__"
...
genome=$(cat ${filePathForTable} | gawk -v i=${SLURM_ARRAY_TASK_ID} 'NR==i{print $1}')
filePathForFasta=$(cat ${filePathForTable} | gawk -v i=${SLURM_ARRAY_TASK_ID} 'NR==i{print $2}')
