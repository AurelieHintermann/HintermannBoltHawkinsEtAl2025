...
filePathForGTF="${genomePath}/mergeOverlapGenesOfFilteredTranscriptsOfMus_musculus.GRCm39.108_ExonsCDSOnly_UCSC.gtf"
...
filePathForSamplesPlan="${gitDir}/RNAseq/samplesplan.txt"
...
if [ ! -e ${dirPathWithResults}/mergedTables/AllCufflinks_Simplified_subset.txt ]; then
...
outputFolderForStep1 <- \"${dirPathWithResults}/mergedTables/\"
...
# remove some genes.
# initialTableWithCount<-'${dirPathWithDependencies}/rnaseq_rscripts/example/mergedTables/AllHTSeqCounts.txt'
...
# normalize it.
# initialTableWithFPKM<-'${dirPathWithDependencies}/rnaseq_rscripts/example/mergedTables/AllCufflinks_Simplified.txt'
...
" > ${configFile}
  ...
  if [ ! $(grep "htseq_count_file" $filePathForSamplesPlan) ]; then
    cat $filePathForSamplesPlan | gawk -v pa=$dirPathWithResults 'BEGIN{print "htseq_count_file"}NR>1{print pa"/allFinalFiles/counts_FPKM/htseqCount_"$1".txt"}' > htseqCol.txt
  ...
  if [ ! $(grep "cufflinks_file" $filePathForSamplesPlan) ]; then
    cat $filePathForSamplesPlan | gawk -v pa=$dirPathWithResults 'BEGIN{print "cufflinks_file"}NR>1{print pa"/allFinalFiles/counts_FPKM/FPKM_"$1".txt"}' > CuffCol.txt
  ...
  rm ${filePathForSamplesPlan}_withPaths
  # copy to GEO:
  mkdir -p ${dirPathWithResults}/toGEO
  cp ${dirPathWithResults}/mergedTables/AllCufflinks_Simplified.txt ${dirPathWithResults}/toGEO/
  cp ${dirPathWithResults}/mergedTables/AllCufflinks_Simplified_subset.txt ${dirPathWithResults}/toGEO/
  cp ${dirPathWithResults}/mergedTables/AllHTSeqCounts_subset.txt ${dirPathWithResults}/toGEO/
  # copy to github
  mkdir -p ${gitDir}/RNAseq/outputs/
  cp ${dirPathWithResults}/mergedTables/AllCufflinks_Simplified.txt ${gitDir}/RNAseq/outputs/
fi

# Then specific plots
Rscript ${gitDir}/RNAseq/RNAseq_plots.R ${gitDir}/RNAseq/outputs/AllCufflinks_Simplified.txt ${gitDir}/RNAseq/outputs/
